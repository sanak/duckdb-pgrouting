...
| npm packages in `assets/`: the DuckDB-Wasm `.wasm` and worker files, the MapLibre worker, and the parts of `index-*.js` / `index-*.css` they supply (DuckDB-Wasm, Apache Arrow, MapLibre GL JS, …) | npm | their own licences, listed in `third-party-licenses.txt` next to the page |
